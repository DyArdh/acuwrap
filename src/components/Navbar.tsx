  ...
  return (
    <>
      <div
        ...
        className="hidden md:block md:fixed left-0 top-0 w-full z-10 ease-in duration-300">
        <div className="px-2 py-2 md:px-10 md:py-2 lg:max-w-[1280px] lg:px-10 lg:py-2 m-auto flex justify-between items-center">
          ...
          <ul
            ...
            {Links.map((link) => (
              <li key={link.name}>
                <a className="hover:text-second duration-500" href={link.url}>
                  {link.name}
                </a>
              </li>
            ))}
              ...
              <li className="flex justify-center flex-col items-center gap-1 text-primary">
                ...
                <li className="flex justify-center flex-col items-center gap-1 text-primary">
                  <svg className="w-6" viewBox="0 0 24 24">
                    <path
                      fill="#00b835" 
                      fill-rule="evenodd" d="M11.943 1.25h.114c2.309 0 4.118 0 5.53.19c1.444.194 2.584.6 3.479 1.494c.895.895 1.3 2.035 1.494 3.48c.19 
                      1.411.19 3.22.19 5.529v.088c0 1.909 0 3.471-.104 4.743c-.104 1.28-.317 2.347-.795 3.235c-.21.391-.47.742-.785 1.057c-.895.895-2.035 1.3-3.48 
                      1.494c-1.411.19-3.22.19-5.529.19h-.114c-2.309 0-4.118 0-5.53-.19c-1.444-.194-2.584-.6-3.479-1.494c-.793-.793-1.203-1.78-1.42-3.006c-.215-1.203-.254-2.7-.262-4.558c-.002-.473-.002-.973-.002-1.501v-.058c0-2.309 
                      0-4.118.19-5.53c.194-1.444.6-2.584 1.494-3.479c.895-.895 2.035-1.3 3.48-1.494c1.411-.19 3.22-.19 5.529-.19Zm-5.33 1.676c-1.278.172-2.049.5-2.618 1.069c-.57.57-.897 1.34-1.069 2.619c-.174 1.3-.176 3.008-.176 5.386v.844l1.001-.877a2.3 2.3 0 0 1 3.141.105l4.29 4.29a2 2 0 0 0 2.564.222l.298-.21a3 3 0 0 1 3.732.225l2.83 2.547c.286-.598.455-1.384.545-2.493c.098-1.205.099-2.707.099-4.653c0-2.378-.002-4.086-.176-5.386c-.172-1.279-.5-2.05-1.069-2.62c-.57-.569-1.34-.896-2.619-1.068c-1.3-.174-3.008-.176-5.386-.176s-4.086.002-5.386.176Z" clip-rule="evenodd" 
                    />
                  </svg>
